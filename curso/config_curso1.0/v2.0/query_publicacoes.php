...
if ($CursoComercial == 1):
    ...
    if ($CursoComercial == 0 and $codigousuario != 1):
        $queryLicoes = $con->prepare("SELECT * FROM a_aluno_publicacoes_cursos, new_sistema_publicacoes_PJA
    WHERE idmodulopc = :idmodulo AND codigopublicacoes = idpublicacaopc
    AND a_aluno_publicacoes_cursos.visivelpc = '1' AND a_aluno_publicacoes_cursos.aulaliberadapc = '1'
    ORDER BY ordempc ASC");
    else:
        $queryLicoes = $con->prepare("SELECT * FROM a_aluno_publicacoes_cursos, new_sistema_publicacoes_PJA
    WHERE idmodulopc = :idmodulo AND codigopublicacoes = idpublicacaopc
...
if ($rwAtual) {
    ...
    $aulaLiberada = $rwAtual['aulaliberadapc'];
    if ($aulaLiberada != '1') {
        $aulaLiberada = '0';
    }
    // Consulta anterior
