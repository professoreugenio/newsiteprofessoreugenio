...
try {
    ...
    // Busca conteúdo + dados do usuário e da aula
    ...
    // Monta HTML visual (equivalente a nl2br(htmlspecialchars()))
    ...
    // Regra da foto
    if (($row['img50'] ?? '') === 'usuario.jpg'):
        $foto = '../../fotos/usuarios/' . ($row['img50'] ?? '');
    else:
    ...
    echo json_encode([
        'ok'       => true,
        'html'     => $html,
        'textoRaw' => (string)($row['textoCF'] ?? ''), // <- adicionado
        'nome'     => (string)($row['nomeUsuario'] ?? 'Aluno'),
        'foto'     => $foto,
        'titulo'   => (string)($row['tituloAula'] ?? '-')
    ]);
} catch (Exception $e) {
